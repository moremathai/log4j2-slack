package com.mmi.log4j2.slack.appender;

import com.mmi.log4j2.slack.message.SlackLogMessage;
import com.mmi.log4j2.slack.model.SlackLog;
...
public class SlackAppender extends AbstractAppender {
	...
	public void append(LogEvent event) {
		if (event.getMessage() instanceof SlackLogMessage) {
			...
			try {
				...
				if (this.username != null) builder.username(username);
	...
	protected Payload convertToPayload(LogEvent event) {
		System.out.println("ConvertToPayload");
		...
		if (channel != null) builder = builder.channel(channel);
		if (iconUrl != null) builder = builder.iconUrl(iconUrl);
